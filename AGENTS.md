...
## WebContainer / Vite 目录约定（新增）

- WebContainer 内的前端业务代码必须统一放在 `src/` 下，禁止再将 `App.tsx`、`main.tsx`、`index.css`、组件、hooks、pages 等业务文件直接放在项目根目录。
- 根目录只允许保留构建与运行时配置文件：`package.json`、`index.html`、`tsconfig.json`、`vite.config.ts`，以及流程文件 `todo.md`。
- `project_files.path` 的规范路径也必须保持与上述目录一致：业务代码使用 `/src/...`，不得继续写入 `/App.tsx`、`/components/...` 这类扁平根路径。
