...
type SidebarDrawerTriggerProps = {
  openSidebar: () => void;
  isPinned: boolean;
...
export function SidebarAndHeader({
  ...
  const currentBalance = resolvedProfile?.ai_balance ?? null;
  const maxBalance = resolvedProfile?.max_ai_balance ?? null;
  const balancePercent =
    currentBalance !== null && maxBalance !== null && maxBalance > 0
      ? Math.round((currentBalance / maxBalance) * 100)
      : 100;
          ...
          <div className="p-3 border-t border-border relative">
            ...
            <div className="px-2 mt-3 mb-1">
              <div className="flex items-center justify-between text-[11px] text-foreground/50 mb-1.5 font-medium">
                <span>额度</span>
                <span>
                  {currentBalance !== null && maxBalance !== null
...
export function SidebarDrawer({
  ...
  profile: UserProfile | null;
  onSidebarChange?: (pinned: boolean) => void;
  renderTrigger?: (props: SidebarDrawerTriggerProps) => React.ReactNode;
}) {
  const [resolvedUser, setResolvedUser] = useState<User | null>(user);
  ...
  const showSidebar = resolvedUser && (isPinned || isHovered);
  const openSidebar = () => setIsHovered(true);

  return (
    <>
      {renderTrigger ? renderTrigger({ openSidebar, isPinned, isSidebarVisible: Boolean(showSidebar) }) : null}
