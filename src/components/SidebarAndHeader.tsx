...
import { Atom, PanelLeftClose, PanelLeft, Settings, LogOut, MessageSquare, MoreHorizontal } from "lucide-react";
import { useTheme } from "next-themes";
import { motion, AnimatePresence } from "framer-motion";
import { getBrowserSupabaseClient } from "@/lib/supabase-browser";

export function SidebarAndHeader({ user, onSidebarChange }: { user: User | null; onSidebarChange?: (pinned: boolean) => void }) {
  ...
  return (
    <>
      ...
      <header className={`fixed top-0 left-0 right-0 h-16 flex items-center justify-between px-4 sm:px-6 transition-all duration-300 z-30 ${isPinned && user ? 'pl-72' : ''}`}>
        <div className="flex items-center gap-4">
          {!user || (!isPinned && !isHovered) ? (
            ...
            >
              ...
              <span className="text-[22px] font-semibold tracking-tight text-foreground">Atoms</span>
            </Link>
          ) : null}
        ...
        >
          <div className="flex items-center justify-between h-16 px-4">
            ...
            >
              ...
              <span className="text-[18px] font-semibold tracking-tight text-foreground truncate">Atoms</span>
            </Link>
            ...
            <div className="flex items-center gap-1 group">
              ...
              >
                {isPinned ? <PanelLeftClose className="size-4.5" /> : <PanelLeft className="size-4.5" />}
              </button>
            </div>
          </div>

          <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1">
            <div className="text-xs font-semibold text-foreground/40 px-2 pt-2 pb-2">最近的项目</div>
            ...
            >
              <div className="flex items-center gap-2.5 overflow-hidden">
                ...
                <div className="flex flex-col overflow-hidden">
                  <span className="text-sm font-medium text-foreground/85 truncate leading-tight">{user.email?.split('@')[0]}</span>
                  <span className="text-[11px] text-foreground/40 truncate leading-tight mt-0.5">Free Plan</span>
                </div>
              </div>
                ...
                <>
                  ...
                  >
                    <div className="px-3 pt-2 pb-3 mb-1 border-b border-foreground/5 flex flex-col">
                      <span className="text-sm font-medium text-foreground/90 truncate">{user.email?.split('@')[0]}</span>
                      <span className="text-xs text-foreground/40 truncate mt-0.5">{user.email}</span>
                    </div>
