...
const getStatusClasses = (status: WorkflowStep["status"]) => {
  if (status === "running") {
          ...
          >
            <div className="px-4 pb-4 pt-1">
              {steps.map((step, index) => {
                ...
                const displayTitle = step.source === "tool" && step.toolName
                ...
                return (
                  <div key={step.id} className="flex gap-3">
                    <div className="flex w-5 flex-col items-center pt-1">
                      <Circle className="h-2.5 w-2.5 fill-current text-border" />
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2 text-[13px] text-foreground/85">
                              <Icon className="mt-0.5 h-4 w-4 shrink-0 text-foreground/55" />
                          ...
                          <span className={`shrink-0 rounded-full border px-2.5 py-1 text-[11px] ${getStatusClasses(step.status)}`}>
                            {step.status}
                          </span>
                        </div>
                      </div>
